...
void moveTower(node **front, int nHeight_Total, int before, int after){
	int flag_num = 0; // for escape
	int loc_from = 0;
	int loc_to = 0;
	int temp = 0;
	int i = 0; // for change after

	// if even height tower
	if ((nHeight_Total % 2) != 1){
	...
	moveOnePiece(front, loc_from, loc_to);
	flag_num = front[loc_to]->num;

	while ((front[before] != NULL) || (front[3 - (before + after)] != NULL)){

		// '1' was moved -> '1' will not moved
		if (flag_num == 1){

			// loc_from is empty -> move block to empty
			if (front[loc_from] == NULL){
				loc_from = 3 - (loc_from + loc_to);
				loc_to = 3 - (loc_from + loc_to);
			}
			
			else{
				// '2' was behind '1' -> move '2'
				if (front[loc_from]->num == 2){
					loc_to = 3 - (loc_from + loc_to);
				}

				// '2' is behind '1' (now) -> move other block
				else if (front[loc_to]->next->num == 2){
					...
					// loc_to is empty -> move block to empty
					else if (front[3 - (loc_from + loc_to)] == NULL){
						loc_to = 3 - (loc_from + loc_to);
					}

					// loc_from & loc_to is not empty 
					else{

						// find smaller block -> move smaller block to the other
						if (front[loc_from]->num < front[3 - (loc_from + loc_to)]->num){
							loc_to = 3 - (loc_from + loc_to);
						}
		...
		// '1' was not moved -> '1' will be moved
		else if (flag_num != 1){

			// '2' was moved
			if (front[loc_to]->num == 2){
				loc_from = 3 - (loc_from + loc_to);
			}

			// '2' was not moved ('1' also)
			else{
				loc_from = 3 - (loc_from + loc_to);
				
				if ((front[loc_to]->num) % 2 != 0){
					loc_to = 3 - (loc_from + loc_to);
				}
			}
		}
